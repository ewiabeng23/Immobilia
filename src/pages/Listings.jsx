...
export default function Listings() {
  ...
  useEffect(() => {
    supabase
      ...
      .then(({ data, error }) => {
        ...
        if (!error && data) setDbProperties(data)
        setLoading(false)
      })
  }, [])
  ...
  const filtered = useMemo(() => {
    ...
    return list
  }, [query, listingType, city, propType, minPrice, maxPrice, bedrooms, sort, dbProperties])
  ...
  const propTypeLabels = PROPERTY_TYPES.reduce((acc, pt) => { acc[pt] = t.prop_types[pt] || pt; return acc }, {})
      ...
      <div className="container">
        ...
        <div className={`${styles.topBar} glass`}>
          <div className={styles.searchWrap}>
            ...
            <input type="text" className={styles.searchInput} placeholder={t.hero.search_placeholder} value={query} onChange={e => setQuery(e.target.value)} />
            {query && <button className={styles.clearBtn} onClick={() => setQuery('')}><X size={14}/></button>}
          </div>
          <div className={styles.typeTabs}>
            {['all','rent','buy','sell'].map(tab => (
              <button key={tab} className={`${styles.typeTab} ${listingType === tab ? styles.typeTabActive : ''}`} onClick={() => setType(tab)}>
                {t.filters[tab]}
              </button>
            ))}
          </div>
          ...
          <button className={`${styles.filterBtn} ${filtersOpen ? styles.filterBtnActive : ''} ${hasFilters ? styles.filterBtnDirty : ''}`} onClick={() => setFilters(o => !o)}>
        ...
        {filtersOpen && (
          <div className={`${styles.filtersPanel} glass fade-in`}>
            ...
            <div className={styles.filterActions}>
              <button className={styles.resetBtn} onClick={resetFilters}><X size={14} /> {t.filters.reset}</button>
            </div>
          </div>
        )}
        ...
        {hasFilters && (
          <div className={styles.chips}>
            ...
            {minPrice && <span className={styles.chip}>Min: {new Intl.NumberFormat('fr-FR').format(minPrice)} <button onClick={() => setMinPrice('')}><X size={11}/></button></span>}
            {maxPrice && <span className={styles.chip}>Max: {new Intl.NumberFormat('fr-FR').format(maxPrice)} <button onClick={() => setMaxPrice('')}><X size={11}/></button></span>}
